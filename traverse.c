...
void loadmark(int initialmark, int loopmark,int length,Xref xref,int org,unsigned int nref, FILE * out)
{
	/*
	unsigned int calc = 0;
...
char *traverse(unsigned char *buffer, Config *config)
{
    ...
    char *str = malloc( strlen(config->filename) + 100 );
	strcpy(str,config->filename);
	...
	for(;;) // add break after x amount of time
	{
		if(passed[ProgramCounter]) // if we have seen this instruction 
		{
			...
			{
				fwrite(data,sizeof(char),config->len,out);
		...
		/*	case 0xb: // sltiu potential jump table start 	
			...
			*/
			...
			case 0x20: // load byte
				loadmark(0,0,length,xref,org,nref,out);
				break;
				
			case 0x23: // load word
				loadmark(-2,-1,length,xref,org,nref,out);		
				break;
