...
// refactoring
// use macros for the switch statemetns
// add more generic decoding functions
// general indentaion and use of {
// put i type in its own function
// add a opcode printing funciton
// return when hitting an invalid opcode so everything is properly freed
// even when the program cant continue disassembly
...
// add 'signed hex printing for loads and arithmetic funcitons'
// add register names for cop0 and cop1 
...
/*---------------------
* func: do_op
* purpose: load our opcode struct fields
----------------------*/
...
{
	op.simm = (op.imm << 2);
	unsigned int jumpfinal = ProgramCounter + op.simm;
...
void disass_mips(unsigned char *buffer,char *data, Config *config)
{
	// factor away later
	unsigned int calc = 0;
		...
		if(data[ProgramCounter] > 0) // if an instruction
		{
			...
			switch(op.instruction)
			{
				...
				case 0x2: // jump
					...
					if( (jumpfinal - org&0xfffffff) < length || config->flag == 2)
					{
						printf("j loc_%x", jumpfinal + (org&0xf0000000));
					}
					else  // pass the real length aswell
					{                          // but not a needed feature
				...
				case 0x11: // cop(floating point co-processor
					//00010000
	...
	if(config->flag != 2) 
	{ // if we are doing xref caller will free // we aernt done yet
		free(data);
	}
}
...
/*-------------------------
* func: cop1
* purpose: disassemble co-processor 1 opcodes
-------------------------*/
